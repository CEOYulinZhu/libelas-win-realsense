...
bool Matrix::solve (const Matrix &M, FLOAT eps) {
  ...
  for (i=0;i<m;i++) {
    ...
    // 现在已经找到主元，如有需要则交换行，使主元落在对角线上。
    // 注意这里只交换行，不真正交换列，只是通过索引重新标记列。
    if (irow != icol) {
      for (l=0;l<m;l++) SWAP(A.val[irow][l], A.val[icol][l])
      for (l=0;l<n;l++) SWAP(B.val[irow][l], B.val[icol][l])
    }
    
    indxr[i]=irow; // 记录当前主元所在行
    indxc[i]=icol; // 记录当前主元所在列
  ...
  // 列消元主循环结束后，还需要根据之前的列交换恢复解的顺序。
  // 这里按照最初构造置换的反向顺序，成对地交换列以“解开”置换。
  for (l=m-1;l>=0;l--) {
    if (indxr[l]!=indxc[l])
      for (k=0;k<m;k++)
...
// 给定矩阵 a[1..n][1..n]，本例程将其替换为按行置换后的 LU 分解形式。
// a 与 n 为输入；a 在输出时按《Numerical Recipes》中公式 (2.3.14) 的形式存储。
// indx[1..n] 为输出向量，记录部分主元选取所产生的行置换；
// d 为输出符号（±1），表示行交换次数是偶数还是奇数。
// 本函数常与回代程序一起使用，以求解线性方程组或求矩阵的逆。
...
bool Matrix::lu(int32_t *idx, FLOAT &d, FLOAT eps) {
  ...
  FLOAT* vv = (FLOAT*)malloc(n*sizeof(FLOAT)); // vv 存储每一行的缩放因子（隐式缩放）
  d = 1.0;
  for (i=0; i<n; i++) { // 遍历各行，计算并记录行缩放信息
    big = 0.0;
    for (j=0; j<n; j++)
      if ((temp=fabs(val[i][j]))>big)
        big = temp;
    if (big == 0.0) { // 若该行没有非零元素，则矩阵为奇异矩阵
      free(vv);
      return false;
    }
    vv[i] = 1.0/big; // 保存该行的缩放系数
  }
  for (j=0; j<n; j++) { // Crout 方法中的按列主循环
    for (i=0; i<j; i++) { // 对 i<j 的情况应用公式（对应《Numerical Recipes》2.3.12）
      sum = val[i][j];
      for (k=0; k<i; k++)
        sum -= val[i][k]*val[k][j];
      val[i][j] = sum;
    ...
    if (j!=imax) { // 如有需要则交换行
      ...
      d = -d;     // 同时改变符号 d（记录交换次数奇偶性）
      vv[imax]=vv[j]; // 相应地交换缩放因子
    }
    idx[j] = imax;
    if (j!=n-1) { // 最后一步：用主元对下面各行进行归一化
      dum = 1.0/val[j][j];
      for (i=j+1; i<n; i++)
        val[i][j] *= dum;
    }
...
// 给定矩阵 M/A[1..m][1..n]，本例程计算其奇异值分解 M/A = U·W·V^T。
// 其中矩阵 U 在输出时替换原矩阵 A；奇异值对角矩阵 W 以向量 w[1..n] 形式输出；
// 矩阵 V（而非其转置 V^T）以 v[1..n][1..n] 形式输出。
void Matrix::svd(Matrix &U2,Matrix &W,Matrix &V) {
  ...
  g = scale = anorm = 0.0; // Householder 变换，将矩阵化为双对角形式
  for (i=0;i<n;i++) {
    l = i+1;
    rv1[i] = scale*g;
    g = s = scale = 0.0;
  ...
  for (i=IMIN(m,n)-1;i>=0;i--) { // 累积左侧正交变换（构造 U）
    l = i+1;
    g = w[i];
  ...
  for (k=n-1;k>=0;k--) { // 将双对角矩阵对角化：遍历每个奇异值
    for (its=0;its<30;its++) { // 对每个奇异值做最多 30 次迭代
      flag = 1;
      for (l=k;l>=0;l--) { // 测试是否可以进行“分裂”（子问题收敛）
        nm = l-1;
        if ((FLOAT)(fabs(rv1[l])+anorm) == anorm) { flag = 0; break; }
        if ((FLOAT)(fabs( w[nm])+anorm) == anorm) { break; }
      }
      if (flag) {
        c = 0.0; // 若 l>1，则将 rv1[l] 消去
        s = 1.0;
        for (i=l;i<=k;i++) {
          f = s*rv1[i];
          rv1[i] = c*rv1[i];
  ...
  // 对奇异值及其对应的 U、V 列按绝对值从大到小排序；
  // 同时根据正负性翻转整列符号，以使正元素尽可能多。
  int32_t s2,inc=1;
  FLOAT   sw;
  ...
  for (k=0;k<n;k++) { // 根据正负性情况，对整列进行符号翻转
    s2=0;
